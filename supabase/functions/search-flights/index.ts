...
const RATE_LIMIT_MAX_REQUESTS = 30; // 30 requests per minute (more conservative for external API calls)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      console.log("Search flights: Rate limit exceeded for user", userId);
      return new Response(
